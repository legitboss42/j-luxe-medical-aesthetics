...
export default function TemplateSkinBoosters({
  ...
  const contraindicationOptions = [
    ...
    "Pregnancy or breastfeeding",
    "Recent injectable treatment in same area",
    "Recent laser resurfacing or chemical peel",
    "Known hypersensitivity to treatment ingredients",
    "None of the above",
  ] as const;
  ...
  const treatmentSelectionOptions = [
    ...
    "Lumi Eye Pro (1 Session)",
    "Lumi Eye Pro (3 Sessions)",
    "Lumi Pro Skinbooster (1 Session)",
    "Lumi Pro Skinbooster (3 Sessions)",
    "Iluma Luna",
    "Sunekos 200 (undereye)",
    "Sunekos 1200",
    ...
    "Profhilo (1 Session)",
    "Profhilo (2 Session)",
    "Jalupro: classic, HMW, Superhydro",
    "Nucleofill & Polynucleotide (1 Treatment)",
    "Polyneucleotide: Plinest, Plenhyage, Newest",
    "PDRN: Rejuran HB, Rejuran Healer",
    "Mesotherapy",
    "Other",
  ] as const;
  ...
  return (
    <>
      <section className="rounded-2xl border border-white/12 bg-black/35 p-5">
        ...
        <label htmlFor="doctorCareBoostersDetails" className={`${labelClassName} mt-4`}>3. If &quot;Yes&quot; please specify</label>
        <textarea id="doctorCareBoostersDetails" name="doctorCareBoostersDetails" rows={3} className={`${inputClassName} resize-none`} />
        ...
        <label htmlFor="conditionsBoostersDetails" className={`${labelClassName} mt-4`}>
          5. If &quot;Yes&quot; to any of these including current medication or &quot;Other Medical Condition&quot;, please specify
        </label>
        <textarea id="conditionsBoostersDetails" name="conditionsBoostersDetails" rows={3} className={`${inputClassName} resize-none`} />
      </section>
      ...
      <section className="rounded-2xl border border-white/12 bg-black/35 p-5">
        ...
        <textarea id="batchBoosters" name="batchBoosters" rows={2} className={`${inputClassName} resize-none`} />
        <label htmlFor="unitsBoosters" className={`${labelClassName} mt-4`}>Treatment Given / Units Used:</label>
        <textarea id="unitsBoosters" name="unitsBoosters" rows={2} className={`${inputClassName} resize-none`} />
        <label htmlFor="postCommentsBoosters" className={`${labelClassName} mt-4`}>Post-Treatment Comments:</label>
        <textarea id="postCommentsBoosters" name="postCommentsBoosters" rows={3} className={`${inputClassName} resize-none`} />
      </section>
    </>
  );
}
